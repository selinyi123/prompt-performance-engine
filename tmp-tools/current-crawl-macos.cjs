...
function sign(pathAndQuery, dc0) {
  const out = execFileSync('./signer/target/release/zhihu-sign-cli', [pathAndQuery, dc0], { encoding: 'utf8' });
  const h = {};
...
async function main() {
  ...
  if (fs.existsSync('MediaCrawler/libs/stealth.min.js')) {
  ...
  let cookies = await context.cookies([HOME]);
  ...
  if (pages > 0) {
    fs.writeFileSync('mac-current-result.json', JSON.stringify({
      source: 'Zhihu articles API with 2026 zhihu_sign v0.1.0 on GitHub macOS runner; MediaCrawler stealth bootstrap',
